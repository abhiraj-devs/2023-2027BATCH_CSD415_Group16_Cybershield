  ...
  return (
    <div className="space-y-6 font-sans pb-10">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-xl p-6 space-y-4 shadow-2xl">
            <div>
              ...
              <h3 className="text-base font-bold text-zinc-100 mt-1">Delete Account Permanently</h3>
            </div>
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2 px-3 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-300 rounded text-xs font-medium transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
          ...
          {user ? (
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
              <button
                ...
                className="flex-1 sm:flex-initial px-3 py-2 rounded-md bg-zinc-900 hover:bg-zinc-800 text-zinc-300 border border-zinc-800 text-xs font-medium transition-colors cursor-pointer text-center"
              >
                Authentication Details
              </button>
              <button
                onClick={logout}
                className="flex-1 sm:flex-initial px-3 py-2 rounded-md bg-zinc-900 hover:bg-zinc-800 text-zinc-300 hover:text-white border border-zinc-800 text-xs font-medium transition-colors cursor-pointer text-center"
              >
                Sign Out
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-md bg-[#111111] border border-zinc-800 space-y-4 flex flex-col">
          <div className="flex items-center justify-between border-b border-zinc-800 pb-2">
            <h3 className="text-sm font-bold text-zinc-100">Identity & Role Status</h3>
            {isAdmin ? (
          ...
          <div className="space-y-4 text-sm font-mono text-zinc-300 flex-1">
            {/* User Details */}
            <div>
              <label className="block text-[10px] text-zinc-500 uppercase tracking-wider mb-1">Authenticated Account</label>
              <div className="p-3 bg-zinc-950 border border-zinc-900 rounded flex items-center space-x-3">
                {user?.photoURL ? (
                  <img src={user.photoURL} alt={user.displayName || "User"} className="w-9 h-9 rounded-full border border-zinc-700 object-cover" referrerPolicy="no-referrer" />
                ) : (
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-bold text-zinc-100 truncate">{user?.displayName || (user ? user.email?.split("@")[0] : "Not Signed In")}</p>
                  <p className="text-[11px] text-zinc-400 truncate">{user?.email || "Sign in using Google or Email/Password + OTP"}</p>
                </div>
              </div>
            </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2 pt-1">
                        <button
                          ...
                          disabled={resending}
                          className="flex-1 px-2.5 py-1.5 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 rounded text-[11px] font-sans text-zinc-200 transition-colors cursor-pointer"
                        >
            ...
            <div>
              ...
              <div className="p-3 bg-zinc-950 border border-zinc-900 rounded flex items-center justify-between gap-3">
        ...
        <div className="p-6 rounded-md bg-[#111111] border border-zinc-800 space-y-4 flex flex-col">
          <div className="border-b border-zinc-800 pb-2">
            <h3 className="text-sm font-bold text-zinc-100">Data Management & Permissions</h3>
          </div>
          <p className="text-sm text-zinc-400 font-sans">
            Manage data retention, purge cached scans, and inspect authorization policies.
          </p>

          <div className="p-3.5 bg-zinc-950 border border-zinc-900 rounded space-y-2 text-xs flex-1">
            <div className="text-zinc-300 font-bold uppercase tracking-wider text-[10px]">
              Permission Matrix
            </div>
